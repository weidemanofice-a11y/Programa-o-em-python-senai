...
def inserir_usuario():
    ...
    if  nome and cpf:
        conn = conectar()
        c = conn.cursor()
        c.execute('INSERT INTO usuarios(nome, cpf, tel, endereco, telcont, procedi) VALUES(?,?,?,?,?,?)', (nome, cpf, tel, endereco, telcont, procedi))
        conn.commit()
        conn.close()
...
def delete_usuario():
    ...
    if dado_del:
       ...
       conn = conectar()
       c = conn.cursor()    
       c.execute('DELETE FROM usuarios WHERE cpf = ? ',(user_id,))
       conn.commit()
       conn.close()
...
def editar():
     ...
     if selecao:
         user_id = tree.item(selecao)['values'][1]
         ...
         if novo_nome and novo_cpf:
            conn = conectar()
            c = conn.cursor()    
            c.execute('UPDATE usuarios SET nome = ? , cpf = ?, tel = ?, endereco = ?, telcont = ?, procedi = ? WHERE cpf = ? ',(novo_nome,novo_cpf,novo_tel,novo_endereco,novo_telcont,novo_procedi,user_id))
            conn.commit()
            conn.close()  
...
label_tel.grid(row=2, column=0, padx=10, pady=10)

entry_tel = tk.Entry(janela, text = 'N° Telefone:')
entry_tel.grid(row=2, column=1, padx=10, pady=10)
...
label_telcont.grid(row=4, column=0, padx=10, pady=10)

entry_telcont = tk.Entry(janela, text = 'Telefone de contato:')
entry_telcont.grid(row=4, column=1, padx=10, pady=10)

label_procedi = tk.Label(janela, text = 'Procedimento:')
label_procedi.grid(row=5, column=0, padx=10, pady=10)

entry_procedi = tk.Entry(janela, text = 'procedimento:')
entry_procedi.grid(row=5, column=1, padx=10, pady=10)


f = tk.Frame(janela)
f.grid(row=6, column=0)

btn_salvar = tk.Button(f, text='Salvar', command=inserir_usuario)
btn_salvar.grid(row=6, column=0, padx=10, pady=10)

btn_deletar = tk.Button(f, text='deletar', command=delete_usuario )
btn_deletar.grid(row=6, column=1, padx=10, pady=10)

btn_atualizar = tk.Button(f, text='atualizar', command=editar)
btn_atualizar.grid(row=6, column=2, padx=10, pady=10)



columns = ('NOME', 'CPF', 'TEL', 'ENDERECO', 'TELCONT', 'PROCEDI')
